...
class InferenceDataset(Dataset):
    ...
    def get(self, idx):
        ...
        complex_graph['receptor'].x = torch.cat([seq_rec.reshape([-1,1]),node_s_rec, torch.from_numpy(lm_embeddings_rec).float()], axis=1) if lm_embeddings_rec is not None else torch.cat([seq_rec.reshape([-1,1]),node_s_rec], axis=1) # [num_res, 1+9+1280]
        ...
        complex_graph['pep'].x = torch.cat([seq_pep.reshape([-1,1]), torch.from_numpy(lm_embeddings_pep).float()], axis=1) if lm_embeddings_pep is not None else seq_pep.reshape([-1,1])
