...
def get_protein_feature_mda(receptor_file, top_k=24, lm_embedding_chains=None):
    ...
    with torch.no_grad():
        ...
        for i, chain in enumerate(protein_mol.segments):
            ...
            for res_idx, residue in enumerate(chain.residues):
                ...
                if len(c_alpha) == 1 and len(n) == 1 and len(c) == 1 and len(o) == 1:
                    ...
                    chain_n_coords.append(n.positions[0].astype(np.float32))
                    chain_c_coords.append(c.positions[0].astype(np.float32))
                    chain_o_coords.append(o.positions[0].astype(np.float32))
                    ...
                    chain_tip_coords.append(tip.positions[0].astype(np.float32))
        ...
        # node features
        seq = torch.from_numpy(np.asarray(seq))  # 残基类型的整数編码
        node_s = torch.from_numpy(np.asarray(node_s, dtype=np.float32))
        # edge features
        c_alpha_coords = torch.from_numpy(c_alpha_coords.astype(np.float32))
        n_coords = torch.from_numpy(n_coords.astype(np.float32))
        c_coords = torch.from_numpy(c_coords.astype(np.float32))
        tip_coords = torch.from_numpy(tip_coords.astype(np.float32))
        ...
        cedist = (
            torch.from_numpy(dis_matx_center[edge_index[0, :], edge_index[1, :]].astype(np.float32)) * 0.1
        ).view(
            -1, 1
        )  # distance between two centers of mass # [n_edges, 1]
        csdist = (
            torch.from_numpy(dis_matx_side_center[edge_index[0, :], edge_index[1, :]].astype(np.float32))
            * 0.1
        ).view(
            -1, 1
