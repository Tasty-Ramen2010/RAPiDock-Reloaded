    ...
    with torch.no_grad():
        ...
        for i,chain in enumerate(peptide_mol.segments):
            ...
            for res_idx, residue in enumerate(chain.residues):
                ...
                residue_coords = res_atoms.positions.astype(np.float32)
                if len(c_alpha) == 1 and len(n) == 1 and len(c) == 1 and len(o) == 1:
                    ...
                    _ += 1
                    chain_c_alpha_coords.append(c_alpha.positions[0].astype(np.float32))
            ...
            ori_coords.append(new_ori.positions.astype(np.float32))
        ...
        return (new_u_content, torch.from_numpy(np.concatenate(ori_coords).astype(np.float32)), torch.from_numpy(np.asarray(coords, dtype=np.float32)), lm_embeddings, seq, all_edge_index,backbone_edge_index,sidechain_edge_index, atom2res_index, atom2resid_index, atom2atomid_index, pep_a_s)
...
def get_updated_peptide_feature(data, device, top_k=24):
    ...
    # Compute intra_residue features using radius graph - Does as CPU then maps back to GPU to avoid Tensor MPS and CPU issues
    cpu_x = data['pep_a'].atom2res_index.to(torch.float).cpu()
    cpu_batch = data['pep_a'].batch.cpu()

    s, t = torch_cluster.radius_graph(
        cpu_x,
        1,
        batch=cpu_batch,
    ...
    # Changes made to avoid Tensor MPS and CPU issues
    cpu_diff = diff.cpu()
    cpu_inverse = inverse.cpu()

    max_vals = scatter_max(cpu_diff, cpu_inverse)[0].to(device)
    min_vals = scatter_min(cpu_diff, cpu_inverse)[0].to(device)
    ...
    # Compute as CPU then map back to GPU to avoid Tensor MPS and CPU issues
    ca_cpu = ca.cpu()
    batch_cpu = data['pep'].batch.cpu()
